...
const AddFilm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validasi session
    if (!userId || !role) {
      alert('Anda harus login untuk menambahkan film.');
      return;
    }

    // Validasi role
    if (role !== 'author') {
      alert('Anda tidak memiliki izin untuk menambahkan film.');
      return;
    }

    // Validasi jumlah genre
    ...
    try {
      const response = await fetch('/api/films', {
        ...
        body: JSON.stringify({
          ...
          created_by: userId, // Gunakan userId dari session
        }),
      });
      ...
      if (response.ok) {
        ...
        alert(errorData.error || 'Terjadi kesalahan saat menambahkan film.');
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      >
        <HiFilm className="w-5 h-5 mr-2" /> {/* Ikon film */}
        Tambah Film
      </button>
